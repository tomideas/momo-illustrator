...
### 改善
- **使用指南侧栏重构 / Docs sidebar cleanup**：删除子项里的 ASCII 树符（`└`、`　└`、`　　　· `），改用 CSS `padding-left` 递增的两层缩进（`nav-sub` 14px、`nav-sub2` 28px）。分组标题降为 uppercase 12px 600 字重 + 字母间距，主项 14px 一级文本色，子项 13/12.5px 次级色，纯缩进与色阶表达层级。
...
### 新增
- **选取相同**：在「颜色检查」下方新增「填充颜色」「描边色」「填色和描边」三个快捷按钮，直接调用 Illustrator 原生「选择相同」（Select Same）命令，沿用 Illustrator 对群组、复合路径、专色与外观的判断规则。可先点击颜色库色块将其设为高亮参考色，再按「填充颜色」或「描边色」查找；临时参考对象会在原生命令执行后立即清除。
...
- **使用指南（site）· 选取相同工具**：新增演示视频 `site/assets/videos/select-same.mp4`（864 KB），置于「两种使用方式」表格前
...
- **使用指南（site）**：网格系统新增演示视频
- **使用指南（site）**：尾端空白检查新增演示视频
- **使用指南（site）**：文字溢出检查新增演示视频
- **使用指南（site）**：功能介绍标题移除脚本版本号 badge，版本仅在文首 page-desc 说明
- **使用指南（site）**：快速上手主面板图改为 `iShot_2026-06-26_00.59.42` 原图直传
...
### 发布
- **Extension v2.125** 源码推送：样式检查 v8.3.1、颜色检查 v1.9.x、画板重排 v1.5.9、颜色库提取修复、Momo Notes v1.11 等（2.109–2.125 累积）

### 文档
- **使用指南（site）**：同步至 v2.125——更新版本号；重写文字样式检查说明（v8.3.x 对话框与输出方式）；补充颜色库「提取」选择工具说明、重新排列全画板列表；Momo Notes 撤销/粗体；标注框规则
- **README**：版本号更新为 v2.125
- **使用指南（site）**：ZXP 拖入安装演示视频更新为用户新录制片段
- **使用指南（site）**：快速上手移除重复的菜单打开说明（安装章节已涵盖）
- **使用指南（site）**：演示视频移除黑色背景与圆角边框（CSS 造成黑边，视频文件未处理）
- **使用指南（site）**：扩充快速上手导语（打开方式、四组功能、操作习惯）；移除画板置灰提示
- **使用指南（site）**：安装步骤截图移除 doc-img 圆角边框，避免 iShot 透明圆角叠出白色色块
- **使用指南（site）**：精简快速上手导语，移除重复的「安装并重启」说明
- **使用指南（site）**：安装步骤截图更新——GitHub Releases 下载 .zxp、ZXP/UXP Installer 下载页改为用户 iShot 原图
- **使用指南（site）**：主面板一览图改为用户 iShot 原图直传
- **使用指南（site）**：GitHub Releases 链接改为新窗口打开

### 移除 / Removed
- **样式检查 v8.3.1**：移除检查后的「样式报告」详细弹窗，改为简洁提示 + 画板图例（同颜色检查）

### 改善 / Improvements
- **样式检查 v8.3.0**：对话框排版与颜色检查统一（输出方式合并为勾选框）
- **颜色检查 v1.9.0**：编号改为分色标注，图例同步配色
- **重新排列 v1.5.9**：画板列表移除 80 项上限，显示全部画板

### 修復 / Fixes
- **颜色库「提取」**：选择工具（V）选中群组/复合路径即可提取，不必直接选择子路径

### 其他（2.109 – 2.124 摘要）
- Momo Notes：Cmd+Z 撤销、表格粗体、白屏修复迭代（v1.10–v1.11）
- 颜色检查：「生成说明」图例、移除详细报告弹窗、移除「合并标注」
- 移除开发工具「清除缓存」误导按钮（v2.116）
