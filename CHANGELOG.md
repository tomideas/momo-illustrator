...
### 新增
- **选择相同**：在「颜色检查」下方新增「填充颜色」「描边色」「填色和描边」三个快捷按钮，直接调用 Illustrator 原生 Select Same 命令，沿用 Illustrator 对群组、复合路径、专色与外观的判断规则。可先点击颜色库色块将其设为高亮参考色，再按「填充颜色」或「描边色」查找；临时参考对象会在原生命令执行后立即清除。
