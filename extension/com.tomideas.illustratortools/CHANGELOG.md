...
### 修復
- **侧栏简化：移除折叠与二级标题**：移除全部 `<button>` toggle 与 `.nav-children` 子项，仅保留一级导航。
...
### 改善
- **使用指南侧栏可折叠分组 / Docs sidebar collapsible groups**：含子项的父级改为 `<button>` 点击切换展开／折叠，默认折叠，状态存入 `localStorage`，滚动到子项时自动展开。CSS `max-height` 动画过渡。

## [2.132] — 2026-07-26

### 改善
- **使用指南侧栏重构 / Docs sidebar cleanup**：删除子项里的 ASCII 树符，改用 CSS `padding-left` 递增的两层缩进（`nav-sub` 14px、`nav-sub2` 28px）。分组标题改为 uppercase 12px 600 字重 + 字母间距，主项 14px 一级文本色，子项 13/12.5px 次级色。

## [2.131] — 2026-07-26

### 改善
- **网格系统 v1.3.5 · 完整设置记忆**：持久化网格类型、三类网格参数、间距、对角线、单位、内边距、颜色、作用范围及导出选项；应用、取消或直接关闭窗口后，重新打开均恢复上次状态。

## [2.130] — 2026-07-26

### 改善
- **网格系统 v1.3.4 · 整组输出**：每个目标生成一个网格顶层组，列、行、边框和对角线保留为内部子组，可在画布中一次选择并整体移动。

## [2.129] — 2026-07-26

### 修復
- **网格系统 v1.3.3 · 作用范围记忆落盘**：作用范围改存独立的 `grid_system_settings.json`，并使用 `current / all / selection` ASCII 值，避免中文范围值令旧偏好写入失败并清空窗口位置文件。

## [2.128] — 2026-07-26

### 修復
- **网格系统 v1.3.2 · 剪切蒙版边界**：作用范围为「已选图形」时，剪切蒙版群组改用蒙版路径的几何边界生成预览与正式网格，不再按蒙版外的隐藏内容范围计算。
- **网格系统 · 作用范围记忆**：记住「当前画板 / 全部画板 / 已选图形」的上次选择，重新打开对话框时自动恢复。

## [2.127] — 2026-07-26

### 新增
- **选取相同**：在「颜色检查」下方新增「填充颜色」「描边色」「填色和描边」三个快捷按钮，直接调用 Illustrator 原生「选择相同」（Select Same）命令。颜色库色块可作为高亮参考色供「填充颜色」或「描边色」查找，临时参考对象执行后自动清除。

### 文档
- **PlayerDebugMode 版本说明**：更正 Illustrator 与 CSXS 的旧版对应说明；按 Adobe 官方兼容表注明 Illustrator 25.3 对应 CEP 11、Illustrator 29.5.1 开始对应 CEP 12，并增加 Windows `reg query` 验证命令。
...
### 移除 / Removed
- **样式检查 v8.3.1 · 详细报告弹窗**：移除检查后的「样式报告」滚动窗口，改为与颜色检查一致的简洁提示（发现样式组数 / 需注意组数 / 混合框数 / 标注图层）。完整清单由画板上的「生成说明」图例承载。一并删除随之失效的 `buildResultText` / `showResultWindow` / `artboardsText` / `nowText` / `pad2`。

## [2.124] — 2026-06-26

### 修復 / Fixes
- **颜色库「提取」用选择工具选不到颜色**：原逻辑只读最外层选中对象的 `.fillColor`，且群组只扫直接子级——用选择工具（V）选到群组/复合路径（如转曲文字）时报「所选对象无填色」，必须用直接选择工具（A）点单个 path 才行。改为**递归查找**：路径 / 网格 / 文字 / 复合路径 / 嵌套群组都钻进去找第一个真实填色，并扫描所有选中对象（不止第一个）。现在用选择工具直接选中即可提取。

## [2.123] — 2026-06-26

### 改善 / Improvements
- **样式检查 v8.3.0 对话框排版与颜色检查统一**：
  - **1. 范围**：移走「只检查可见文字 / 包含画板外文字」两个勾选（保留列表多选 + 画板范围 + 全部 + 模式全新/延续 + 说明）。
  - **3. 标记规则 + 4. 输出 → 合并为「3. 输出方式」**：与颜色检查同款布局——「只检查可见对象（跳过隐藏图层）」「包含画板外文字」「红框标出出现次数 ≤ N 次的样式」「生成说明」。原下拉「仅盘点/标记≤N次」改为勾选框（勾上=标记，对应 attentionMode 1/0），逻辑不变。
  - ⚠️「模式：全新/延续」及其编号延续/存档逻辑**完全未改动**。
  - 注：阈值后缀用「次的样式」（图片里写的「颜色」是从颜色检查复制的笔误，样式检查标的是样式）。

## [2.122] — 2026-06-26

### 移除 / Removed
- **颜色检查 v1.9.1「合并标注」**：实测效果不理想（密集散稿场景仍不够清爽），整体移除。一并删除 `topLevelGroup` / `objKey` 及去重逻辑，`drawMarkers` 去掉不再使用的 `abIndexes` 参数。恢复为每个对象单独标注（配合 v1.9.0 的分色编号 + 图例，已不需要合并）。

## [2.121] — 2026-06-26

### 改善 / Improvements
- **颜色检查 v1.9.0 编号配色 / colored labels**：画板标注的颜色编号改为**每个编号一种区分色**（12 色配色板，白底可读），密集区域相邻数字一眼可分。图例「生成说明」的对应行文字用同一套色，画板上的「绿色 3」可对上图例的「绿色 3.」。原先全部蓝色（仅需注意标红）难以区分，已替换。

## [2.120] — 2026-06-25

### 改善 / Improvements
- **颜色检查 v1.8.2**：
  - **「合并标注」对未编组稿件也生效**：实测发现密集包装稿（转曲文字等）的对象往往**根本未编组**，原「按组标注」对其无效（每个路径仍一个编号）。改进去重规则——对象在 group 中则按最外层 group 去重；**未编组对象则按「同色 + 同画板」去重**，使散落的同色路径在每个画板上只标一个编号。复选框更名「合并标注（同组、或同色同画板只标一个编号）」。
  - **对话框标题** 改为「颜色检查」，与面板按钮一致（原为「颜色」）。
  - **移除「标注颜色编号」复选框**：颜色编号标注改为始终开启（必选），去掉冗余开关与说明文字。

## [2.119] — 2026-06-25

### 修復 / Fixes
- **颜色检查 v1.8.1「按组标注」去重失效 / by-group dedup not working**：v1.8.0 用 `===` 比较两个 group DOM 对象判断是否同组，但 ExtendScript 中 DOM 对象的 `===` 不可靠（每次取 `.parent` 可能返回新包装对象，恒为 false），导致去重从未生效、仍每对象一个编号。改用稳定字符串 key（优先 `uuid`，退而用 类型+包围盒）去重。⚠️ 注意：若稿件本身未编组（每个路径都是图层直接子级），按组标注无法减少标注——那种情况需改用「只标稀有色」方案。

## [2.118] — 2026-06-25

### 移除 / Removed
- **颜色检查 · 详细报告弹窗**：移除检查后弹出的「颜色报告」滚动窗口（`showResultWindow`）。完整颜色清单已由画板上的「生成说明」图例承载，报告窗口冗余。一并删除随之失效的 `buildReport` / `artboardsText` / `pad2`。检查后仍保留简洁 alert（发现颜色种数 / 需注意数 / 标注图层）。

## [2.117] — 2026-06-25

### 新增 / Added
- **颜色检查 v1.8.0「按组标注」/ Color Check label-by-group**：新增「按组标注（每组只标一个编号）」勾选框（默认开）。对密集矢量稿（文字转曲、图标由大量小路径组成），原本每个对象都打编号会满屏数字、无法使用；勾选后同一最外层 group 内的同色对象只在 group 范围标注一个编号 + 一个红框，大幅减少视觉杂乱。`topLevelGroup()` 向上查最外层 GroupItem，对象不在组中则按自身标注。

## [2.116] — 2026-06-25

### 移除 / Removed
- **开发工具「清除缓存（白屏修复）」按钮**：经 CEP 日志排查，白屏真因是 `GetExtensionScaleFactor()` 在本机多屏 HiDDPI + Luna Display 虚拟显示器环境下恒返回 0（Adobe 自家面板亦中招），与缓存无关。清缓存"修好"白屏其实是顺带的"重启"起作用。该按钮的"白屏修复"承诺会误导排查方向，整个移除。CEP 缓存陈旧问题改由开发手动处理（清 `~/Library/Caches/CSXS/cep_cache`）。

## [2.115] — 2026-06-25

### 回退 / Reverted
- **撤销 2.113 的 CEFCommandLine（`--disable-gpu` 等）/ Revert CEF GPU flags**：实测发现该改动**让原本正常的主面板也白屏**——调试端口确认两个面板都是「DOM 完整、已布局、未绘制」。环境为 CEP 12 / Chromium 99，`--enable-begin-frame-scheduling` 是 Chromium 40 时代老参数，配合 `--disable-gpu` 疑似破坏新版合成器。完全移除两个面板的 CEFCommandLine，恢复到 2.112 的已知可用状态。白屏根因仍待进一步排查（不再用未验证的全局渲染参数赌一把）。

## [2.114] — 2026-06-25

### 新增 / Added
- **颜色检查 v1.7.0「生成说明」/ Color Check legend**：移植自「文字样式检查」的图例功能。新增「生成说明」勾选框（默认开），在首画板上方（画板外、白底）生成一块颜色编号说明——标题 + 检查类型 + 每个颜色一行（真实颜色色块 + 「编号. 颜色标签 × 出现次数」，需注意的标红）。独立图层「颜色说明」，勾选「运行前删除旧标注图层」时一并清除旧说明。

## [2.113] — 2026-06-25

### 修復 / Fixes
- **Momo Notes 随机白屏 · 根因修复 / Notes random white screen · root-cause fix**：调研确认白屏是 **CEF GPU 合成层的时序竞争**（调试端口现场抓取：HTML 已加载、DOM 完整、零 JS 报错，纯粹"渲染了没刷到屏幕"；症状随机、需多次重启时好时坏，正是 GPU 进程启动状态不定所致）。manifest 两个面板的 `<Resources>` 新增 `<CEFCommandLine>`，加入 `--disable-gpu`、`--disable-gpu-compositing`、`--enable-begin-frame-scheduling`，关闭 GPU 硬件合成、改用纯软件渲染，从根上消除 GPU 时序竞争。文字面板性能损耗无感；如需还原删除 CEFCommandLine 即可。两面板共用同一 CEF 进程，故参数保持一致。

## [2.112] — 2026-06-25

### 改善 / Improvements
- **重新排列 v1.5.9 / Relayout v1.5.9**：移除画板选择列表的 80 项数量上限，改为显示全部画板（不再仅靠「范围」输入选取超出部分）。⚠️ 风险：ScriptUI `listbox` 塞入数百/上千项是已知的潜在卡顿/卡死点，用户已知情并要求去掉上限，需在大量画板（如 594 个）的真实文件中实测确认对话框打开速度可接受。

## [2.111] — 2026-06-25

### 修復 / Fixes
- **Momo Notes 白屏（渲染層）/ Notes white screen (paint layer)**：用调试端口（8089）连进白屏现场实测确认——本次崩溃**没有任何 JS 报错**，HTML 已正常加载完成，纯粹是 CEF 渲染层偶尔没把已渲染好的内容刷到屏幕上（与历史 CHANGELOG v2.65/v2.83/v2.102 同属一类 CEP 已知 timing bug，非本次改动引入）。新增 `visibilitychange`/`focus` 监听，面板重新可见时对 `body.style.zoom` 做极小幅度改动再还原，强制 CEF 重新合成画面（note 面板版本 → 1.11）

## [2.110] — 2026-06-25

### 修復 / Fixes
- **Momo Notes 白屏 / Notes white screen**：note.html 顶部新增全局安全网（`window.onerror` 分段重试 `initUI`），堵住「白屏自愈代码注册前先抛错」导致永久白屏的缺口（note 面板版本 → 1.10）
- **Momo Notes Markdown 表格粗体 / Table bold**：单元格内 `**粗体**` 现正确渲染为 `<strong>`，不再显示字面星号

### 新增 / Added
- **开发工具 → 清除缓存（白屏修复）**：三击版本号展开的开发工具组新增按钮，递归删除 `~/Library/Caches/CSXS/cep_cache` 下所有 `com.tomideas` 缓存目录；CEP 缓存是面板白屏头号元凶，清除后重启 Illustrator 即可修复
- **`.debug`**：补上 note 面板调试端口（8089），便于排查 Notes 面板问题

### 移除 / Removed
- **开发工具**：移除「位置诊断」「库存诊断」两个按钮入口（jsx 脚本保留，便于日后恢复）

## [2.109] — 2026-06-24

### 修復 / Fixes
- **Momo Notes 无法 Cmd+Z 撤销 / Undo (Cmd+Z) not working in Momo Notes**：Illustrator 主程序会把 Cmd+Z / Cmd+Shift+Z / Cmd+Y 当作自己的「撤销/重做上一步操作」快捷键优先拦截，根本不会传到面板网页层，导致笔记里打字后撤销没有反应。改用 CEP `registerKeyEventsInterest` 让面板向 AI 声明接管这些快捷键，并加一层 `execCommand("undo"/"redo")` 的明确处理作保险。
- **Undo (Cmd+Z) not working in Momo Notes**: Illustrator's host app was intercepting Cmd+Z/Shift+Z/Y as its own undo/redo before they reached the panel's webview. Fixed via CEP's `registerKeyEventsInterest` so the panel claims these shortcuts, plus an explicit `execCommand` fallback.
